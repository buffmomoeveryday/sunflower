...
const transformAniListData = (anilistData) => {
	return anilistData.map((anime) => ({
		...
		release_date: anime.startDate
			...
			: '',
		first_air_date: anime.startDate
			? `${anime.startDate.year}-${String(anime.startDate.month || 1).padStart(2, '0')}-${String(anime.startDate.day || 1).padStart(2, '0')}`
			: '',
...
export async function GET(event) {
	...
	try {
		...
		if (type === 'anime') {
			...
			suggestions =
				anilistData.data?.Page?.media?.map((anime) => anime.title.english || anime.title.romaji) ||
				[];
		} else {
			...
			suggestions = tmdbData.results?.slice(0, 5).map((item) => item.title || item.name) || [];
		}
