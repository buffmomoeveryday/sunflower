<script>
  ...
  import { ChevronLeft, ChevronRight, Play, Calendar, Star, Clock, Info, Menu, X, Heart } from 'lucide-svelte';
  ...
  <div class="flex flex-1 overflow-hidden">
    ...
    `}>
      <div class="flex flex-col h-full">
        ...
        <div class="flex-1 p-4 overflow-y-auto">
          <!-- Audio Selection -->
          <div class="mb-6">
            <h3 class="flex items-center gap-2 mb-3 text-sm font-bold text-white uppercase tracking-wide">
              <Play size={16} />
              Audio
            </h3>
          ...
          <div class="mb-4">
            ...
            <div class="grid grid-cols-5 gap-2 max-h-[calc(100vh-200px)] overflow-y-auto no-scrollbar">
    ...
    <div class="flex-1 flex flex-col md:w-3/4 lg:w-4/5 min-h-0">
      ...
      <div class="flex-1 flex flex-col p-4 md:p-6 min-h-0">
        ...
        <!-- Player Controls -->
        <div class="flex flex-wrap items-center justify-between gap-4 p-4 bg-gray-900 rounded-lg flex-shrink-0">
          <!-- Server Selection -->
      ...
      <div class="p-4 md:p-6 space-y-6 overflow-y-auto flex-shrink-0 max-h-64 no-scrollbar">
        ...
        <div class="p-6 bg-gray-900 rounded-lg">
          <h3 class="flex items-center gap-2 mb-4 text-xl font-bold">
            <Info size={20} />
            About {anime_data.title.userPreferred || anime_data.title.romaji || anime_data.title.english}
          </h3>
          <p class="text-gray-300 mb-6 leading-relaxed">
            {@html (anime_data.description ? anime_data.description.replace(/<br\s*\/?>/gi, ' ') : 'No description available.')}
          </p>
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div>
              ...
              <div class="flex items-center gap-2">
                ...
                <span class="text-white font-medium">{anime_data.averageScore ? (anime_data.averageScore / 10).toFixed(1) : 'N/A'}</span>
          ...
          <div class="mt-6">
            ...
            <div class="flex flex-wrap gap-2">
              {#each anime_data.genres as genre}
                <span class="px-3 py-1 text-sm bg-gray-800 border border-gray-700 rounded-full text-gray-300">
                  {genre}
                </span>
              {/each}
            </div>
                  ...
                  <div class="bg-gray-800 rounded-lg overflow-hidden hover:bg-gray-700 transition-colors">
                    {#if relation.coverImage?.medium}
                      <img
                        ...
                        alt={relation.title?.userPreferred || relation.title?.english || relation.title?.romaji || 'Anime Cover'}
                    ...
                    <div class="p-3">
                      <p class="text-sm font-medium line-clamp-2">
                         {relation.title?.userPreferred || relation.title?.english || relation.title?.romaji || 'Unknown Title'}
                      </p>
                    </div>
                  </div>
                </a>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
              {#each anime_data.characters.edges.slice(0, 12) as charEdge}
                <div class="bg-gray-800 rounded-lg overflow-hidden hover:bg-gray-700 transition-colors">
                  ...
                  {/if}
                  <div class="p-3">
                    <p class="text-sm font-medium line-clamp-2">{charEdge.node?.name?.full || 'Unknown'}</p>
