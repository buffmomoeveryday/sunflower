<script>
	...
	import { Heart, ChevronLeft, ChevronRight, Play, Calendar, Star, Users, Clock, Info, Menu, X } from 'lucide-svelte';
...
<div class="flex flex-col min-h-screen text-white bg-black">
	...
	<div class="flex flex-1 overflow-hidden">
						...
						<div class="space-y-2 max-h-96 overflow-y-auto">
							{#each episodes as episode, index}
								<button
									...
									class={`w-full p-3 text-left text-sm transition-all duration-200 rounded-lg border ${
									...
									title={new Date(episode.air_date) > new Date() ? 'This episode is not live yet!' : ''}
								>
									<div class="flex items-center justify-between">
										...
										{#if selectedEpisode === index + 1}
											<Play size={16} class="flex-shrink-0 ml-2" />
										{/if}
									</div>
								</button>
							{/each}
		...
		<div class="flex-1 flex flex-col md:w-3/4 lg:w-4/5 min-h-0">
			...
			<div class="p-4 md:p-6 space-y-6 overflow-y-auto flex-shrink-0 max-h-64">
				<!-- Series Details -->
				<div class="p-6 bg-gray-900 rounded-lg">
					<h3 class="flex items-center gap-2 mb-4 text-xl font-bold">
						<Info size={20} />
						About {seriesDetailData.original_name}
					</h3>
					...
					<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
						<div>
							...
							<div class="flex items-center gap-2">
								...
								<span class="text-white font-medium">{seriesDetailData.vote_average.toFixed(1)}</span>
				...
				{#if episodes[selectedEpisode - 1]}
					<div class="p-6 bg-gray-900 rounded-lg">
						...
						<p class="text-gray-300 mb-4 leading-relaxed">{episodes[selectedEpisode - 1].overview}</p>
						...
						<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
							...
							<div>
								...
								<div class="flex items-center gap-2">
									<Star size={16} class="text-yellow-500" fill="currentColor" />
									<span class="text-white font-medium">{episodes[selectedEpisode - 1].vote_average.toFixed(1)}</span>
									<span class="text-gray-400">/ 10</span>
								</div>
							</div>
						</div>
