...
<div class="flex flex-col items-center justify-center min-h-screen p-4 text-white bg-black">
	<!-- Search Form -->
	<form onsubmit={searchMovie} class="flex w-full max-w-md gap-4 mb-6">
		<input
			type="text"
			bind:value={search}
			name="search"
			...
			required
			class="flex-grow p-2 text-black border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
		/>
		<button
			type="submit"
			class="flex items-center justify-center p-2 text-black bg-white rounded-lg"
			disabled={loading}
		>
			Search
			{#if loading}
	...
	>
		{#each movieResults.filter((movie) => movie.poster_path) as movie}
			<div class="p-2">
				<MovieCard {...movie} class="w-full h-auto" />
			</div>
		{/each}
	</div>
	...
	<!-- Pagination Controls -->
	{#if movieResults.length > 0 && totalPages > 1}
