...
export async function POST({ request, cookies }) {
	const { user_id, tmdb_id, season_id, episode_id, title, poster_path, average_ratings } =
		await request.json();
...
export async function DELETE({ request }) {
	...
	try {
		...
		const series = await pb
			.collection('favourite_series')
			.getFirstListItem(`tmdb_id="${id}" && user_id="${user_id}"`);
		if (!series) {
...
export async function PATCH({ request }) {
	try {
		...
		if (
			!requestData.user_id ||
			!requestData.tmdb_id ||
			!requestData.season_id ||
			!requestData.episode_id
		) {
			return json({ success: false, error: 'Missing required fields' }, { status: 400 });
		}
		const record = await pb
			.collection('favourite_series')
			.getFirstListItem(`tmdb_id="${requestData.tmdb_id}" && user_id="${requestData.user_id}"`);
		if (!record) {
