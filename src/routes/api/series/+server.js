...
export async function POST(event) {
    try {
        ...
        const url = `${BASE_URL_SEARCH}/search/tv?query=${encodeURIComponent(search)}&include_adult=false&language=en-US&page=${page}&api_key=${API_KEY}`; const searchResults = await fetchWithCache(url, `${search}_page${page}`);
